package com.aven.boyrun.material;
...
public class UIModel
{
	...
	public UIModel(ScreenAttribute screenAttribute, int addVelocity)
	{
		...
		mRole = new Role((screenAttribute.maxX - ROLE_ATTRIBUTE_WIDTH) / 2,
				screenAttribute.maxY * 3 / 4, ROLE_ATTRIBUTE_WIDTH, ROLE_ATTRIBUTE_HEITH,
				ROLE_ATTRIBUTE_FRAME_DELAY);
		mRoleVelocityY = GAME_ATTRIBUTE_GRAVITY_VELOCITY;
		mFootboardList = new LinkedList<Footboard>();
		mFootboardList.add(new Footboard((screenAttribute.maxX - BORDER_ATTRIBUTE_IMAGE_WIDTH) / 2,
				screenAttribute.maxY, BORDER_ATTRIBUTE_IMAGE_WIDTH, BORDER_ATTRIBUTE_IMAGE_HEITH,
				FOOTBOARD_TYPE_NORMAL, 1, 1));
	...
	private void generateFootboard()
	{
		...
		mFootboardList.add(new Footboard(mRan.nextInt(mScreenAttribute.maxX
				- BORDER_ATTRIBUTE_IMAGE_WIDTH), mScreenAttribute.maxY + ROLE_ATTRIBUTE_HEITH,
				BORDER_ATTRIBUTE_IMAGE_WIDTH, BORDER_ATTRIBUTE_IMAGE_HEITH, frameType, frameAmount,
				frameDelay));
	}
	...
	private void generateFood()
	{
		...
		// 食物出现的坐标
		mCurFood.mMinX = mRan.nextInt((mScreenAttribute.maxX - FOOD_ATTRIBUTE_IMAGE_SIZE));
		mCurFood.mMinY = mRan.nextInt((mScreenAttribute.maxY - FOOD_ATTRIBUTE_IMAGE_SIZE));
	...
	private void handleBorder()
	{
		...
		if (mRole.getMinY() <= mScreenAttribute.minY)
		{
			...
			{
				mRole.addY(BORDER_ATTRIBUTE_IMAGE_HEITH * GAME_ATTRIBUTE_PIXEL_DENSITY_Y);
			}
			mRoleVelocityY = GAME_ATTRIBUTE_GRAVITY_VELOCITY;
		...
		for (Footboard footboard : mFootboardList)
		{
							...
							.getMaxX()))
			{
				...
				} else
				{
					...
					mScore += mLevel;
					mRoleVelocityY = mFootboartVelocity;
					role.setVirtualY(footboard.getVirtualY() - ROLE_ATTRIBUTE_HEITH
