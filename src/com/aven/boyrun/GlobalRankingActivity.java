...
public class GlobalRankingActivity extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		// 显示进度，并设置取消事件，取消进度时，随即退出当前activity
		mProgressDialog = ProgressDialog.show(this, "",
				getResources().getString(R.string.ranking_title_loading_info), true, true,
		...
		// RankingDataThread是自定义的继承thread的线程类
	...
	class RankingDataThread extends Thread
	{
		...
		{
			mRankingDatas.clear();
			SharedPreferences settings = getSharedPreferences(ConstantInfo.PREFERENCE_RANKING_INFO,
					0);
			String uid = settings.getString(ConstantInfo.PREFERENCE_KEY_RANKING_UID, "");
			// 用HashMap来存储每个玩家的积分等信息
			Map<String, Comparable> myRecord = new HashMap<String, Comparable>();
			myRecord.put("index", "N/A");
			myRecord.put("name",
					settings.getString(ConstantInfo.PREFERENCE_KEY_RANKING_NAME, "Unknow"));
			myRecord.put("score", settings.getInt(ConstantInfo.PREFERENCE_KEY_RANKING_SCORE, 0));
			myRecord.put("bizDate",
					settings.getString(ConstantInfo.PREFERENCE_KEY_RANKING_DATE, "Unknow"));

			Map<?, ?> curObj = null;
			List<Map<String, Comparable>> rankList = getRankingInfo(uid);
			if (rankList != null && rankList.size() > 0)
			{
				...
				if (!isInTopFlag)
				{
					...
					if ((Long) curObj.get("score") >= (Integer) myRecord.get("score"))
					{
	 ...
	 * @return 排名列表
	 */
	public List<Map<String, Comparable>> getRankingInfo(String uuid)
	{
		...
		{// 联网获取积分信息
			...
			if (resultInfo != null && !"ERROR".equals(resultInfo))
			{
				...
				return (List<Map<String, Comparable>>) reader.read(resultInfo);
			}
			dis.close();
		} catch (Exception e)
		{
