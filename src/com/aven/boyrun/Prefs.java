...
public class Prefs extends Activity implements OnClickListener, SeekBar.OnSeekBarChangeListener
{
	...
	public void onCreate(Bundle icicle)
	{
		...
		// 根据配置信息显示当前震动开启与否
		vibrateCheckbox.setChecked(mBaseSettings.getBoolean(ConstantInfo.PREFERENCE_KEY_VIBRATE,
				true));
		...
		vibrateCheckbox.setOnCheckedChangeListener(new CheckBox.OnCheckedChangeListener()
		{
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked)
			{
				if (isChecked)
				{
					mBaseSettings.edit().putBoolean(ConstantInfo.PREFERENCE_KEY_VIBRATE, true)
							.commit();
				} else
				{
					mBaseSettings.edit().putBoolean(ConstantInfo.PREFERENCE_KEY_VIBRATE, false)
							.commit();
				}
			}
		});
		...
		soundsCheckbox.setChecked(mBaseSettings
				.getBoolean(ConstantInfo.PREFERENCE_KEY_SOUNDS, true));
		soundsCheckbox.setOnCheckedChangeListener(new CheckBox.OnCheckedChangeListener()
		{
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked)
			{
				if (isChecked)
				{
					mBaseSettings.edit().putBoolean(ConstantInfo.PREFERENCE_KEY_SOUNDS, true)
							.commit();
				} else
				{
					mBaseSettings.edit().putBoolean(ConstantInfo.PREFERENCE_KEY_SOUNDS, false)
							.commit();
				}
			}
		});
		...
		showTipsCheckbox.setChecked(mBaseSettings.getBoolean(ConstantInfo.PREFERENCE_KEY_SHOWTIPS,
				true));
		showTipsCheckbox.setOnCheckedChangeListener(new CheckBox.OnCheckedChangeListener()
		{
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked)
			{
				if (isChecked)
				{
					mBaseSettings.edit().putBoolean(ConstantInfo.PREFERENCE_KEY_SHOWTIPS, true)
							.commit();
				} else
				{
					mBaseSettings.edit().putBoolean(ConstantInfo.PREFERENCE_KEY_SHOWTIPS, false)
							.commit();
				}
			}
		});
		...
		bestRecordTextView.setText("" + bestRecordTextView.getText()
				+ mRankingSettings.getInt(ConstantInfo.PREFERENCE_KEY_RANKING_SCORE, 0));
	...
	public void onClick(View v)
	{
		...
		{//这里的写法比较有新意，去除字符串后还要替换其中的“/n”为空格
		...
		//将信息存入积分排名配置文件，提交
		mRankingSettings.edit().putString(ConstantInfo.PREFERENCE_KEY_RANKING_NAME, userName)
				.commit();
		switch (v.getId())
		{
	...
	{
		mBaseSettings.edit().putInt(ConstantInfo.PREFERENCE_KEY_POWER, seekBar.getProgress())
				.commit();
	}
